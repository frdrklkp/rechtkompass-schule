/**
 * LDG NRW – Disziplinargesetz für das Land Nordrhein-Westfalen
 * (Landesdisziplinargesetz). Regelt Disziplinarverfahren bei Dienstvergehen
 * von Beamtinnen und Beamten - zentral für Fälle zu Fehlverhalten von
 * Lehrkräften.
 *
 * recht.nrw.de liefert dieses Dokument über dieselbe "LRGV"-Seitenvorlage
 * wie das VwVfG NRW und das LBG NRW (siehe vwvfgNrwParser.ts, dessen
 * Kernlogik hier unverändert übernommen wird). Nur canParse() und die
 * dokumentspezifischen Metadaten unterscheiden sich.
 */
...
const FALLBACK_TITLE = "Disziplinargesetz für das Land Nordrhein-Westfalen (Landesdisziplinargesetz - LDG NRW)";

export const ldgNrwParser: LegalImportParser = {
  id: "ldg-nrw",
  label: "LDG NRW",
  kind: "law",
  ...
  canParse(input: LegalImportInput): boolean {
    ...
    if (input.hint?.officialUrl?.includes("recht.nrw.de") && /disziplinargesetz/i.test(input.hint.officialUrl)) {
      return true;
    }
    return /Disziplinargesetz für das Land Nordrhein-Westfalen|Landesdisziplinargesetz/i.test(raw);
  },
