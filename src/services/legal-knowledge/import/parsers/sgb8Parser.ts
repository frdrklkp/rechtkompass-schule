 ...
 * Bundesrecht; § 8a (Schutzauftrag bei Kindeswohlgefährdung) ist die
 * zentrale Norm für Schulfälle mit Jugendamts-/Kindeswohlbezug (Fund
 * Nachtlauf 2026-08-26: mehrere rote Fälle verweisen auf das fehlende
 * SGB VIII).
 *
 ...
 * übernommen wird). Die Kapitel des SGB VIII tragen ausgeschriebene
 * Ordnungswörter ("Erstes Kapitel") - sie matchen bewusst nicht und die
...
const FALLBACK_TITLE = "Sozialgesetzbuch (SGB) - Achtes Buch (VIII) - Kinder- und Jugendhilfe (SGB VIII)";

export const sgb8Parser: LegalImportParser = {
  id: "sgb-8",
  label: "SGB VIII",
  kind: "law",

  canParse(input: LegalImportInput): boolean {
    if (input.hint?.officialUrl?.includes("gesetze-im-internet.de/sgb_8")) return true;
    const raw = input.raw.slice(0, 4000);
    return /Achtes Buch.*Kinder-?\s*und Jugendhilfe|SGB VIII/i.test(raw);
  },
