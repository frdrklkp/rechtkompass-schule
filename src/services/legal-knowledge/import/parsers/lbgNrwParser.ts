/**
 * LBG NRW – Gesetz über die Beamtinnen und Beamten des Landes
 * Nordrhein-Westfalen (Landesbeamtengesetz).
 *
 * recht.nrw.de liefert dieses Dokument über dieselbe "LRGV"-Seitenvorlage
 * wie das VwVfG NRW (siehe vwvfgNrwParser.ts, dessen Kernlogik hier
 * unverändert übernommen wird - Fassungs-Sidebar, Stammnorm-Kopfzeile,
 * "§ N" auf eigener Zeile, dieselbe Paragraphen-Werkzeugleiste). Nur
 * canParse() und die dokumentspezifischen Metadaten unterscheiden sich.
 */
...
const FALLBACK_TITLE = "Gesetz über die Beamtinnen und Beamten des Landes Nordrhein-Westfalen (Landesbeamtengesetz - LBG NRW)";

export const lbgNrwParser: LegalImportParser = {
  id: "lbg-nrw",
  label: "LBG NRW",
  kind: "law",
  ...
  canParse(input: LegalImportInput): boolean {
    ...
    if (input.hint?.officialUrl?.includes("recht.nrw.de") && /landesbeamtengesetz/i.test(input.hint.officialUrl)) {
      return true;
    }
    return /Gesetz über die Beamtinnen und Beamten des Landes Nordrhein-Westfalen|Landesbeamtengesetz/i.test(raw);
  },
