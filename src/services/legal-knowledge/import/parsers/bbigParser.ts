/**
 * BBiG – Berufsbildungsgesetz. Bundesrecht; regelt das duale Ausbildungs-
 * verhältnis (Pflichten von Auszubildenden und Ausbildenden, Freistellung
 * für den Berufsschulunterricht, Kündigung, Prüfungswesen der Kammern) -
 * für ein Berufskolleg-Werkzeug die zentrale ausbildungsrechtliche Quelle
 * (Fund 2026-09-01: von generierten Azubi-Fällen gebraucht, fehlte komplett).
 *
 * gesetze-im-internet.de liefert dieses Dokument über dieselbe Seitenvorlage
 * wie BeamtStG/KunstUrhG (siehe beamtstgParser.ts); Kapitel mit
...
const FALLBACK_TITLE = "Berufsbildungsgesetz (BBiG)";

export const bbigParser: LegalImportParser = {
  id: "bbig",
  label: "BBiG",
  kind: "law",

  canParse(input: LegalImportInput): boolean {
    if (input.hint?.officialUrl?.includes("gesetze-im-internet.de/bbig")) return true;
    const raw = input.raw.slice(0, 4000);
    return /Berufsbildungsgesetz|BBiG/i.test(raw);
  },
