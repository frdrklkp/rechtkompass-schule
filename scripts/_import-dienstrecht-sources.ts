/**
 * Import der drei zentralen Dienstrecht-Gesetze (Priorität 1, Nutzergespräch
 * 2026-08-25): LBG NRW, BeamtStG, LDG NRW. Nutzt denselben Crawl-Endpunkt
 * und dasselbe Upsert-Muster wie scripts/_import-bass-all.ts, aber mit
 * explizit zugeordnetem Parser je Quelle statt Auto-Erkennung (die drei
 * neuen Parser sind eigens für diese Dokumente gebaut, keine Mehrdeutigkeit).
 *
 * Aufruf: bun run scripts/_import-dienstrecht-sources.ts
 */
import { createClient } from "@supabase/supabase-js";
import { lbgNrwParser, ldgNrwParser, beamtstgParser } from "../src/services/legal-knowledge/import";
...
const TARGETS: Array<{ url: string; label: string; sourceId: string; parser: LegalImportParser }> = [
  {
    url: "https://recht.nrw.de/lrgv/gesetz/04082026-landesbeamtengesetz-lbg-nrw/",
    label: "LBG NRW",
    sourceId: "lbg-nrw",
    parser: lbgNrwParser,
  },
  {
    url: "https://www.gesetze-im-internet.de/beamtstg/BJNR101000008.html",
    label: "BeamtStG",
    sourceId: "beamtstg",
    parser: beamtstgParser,
  },
  {
    url: "https://recht.nrw.de/lrgv/gesetz/05122018-disziplinargesetz-fuer-das-land-nordrhein-westfalen-landesdisziplinargesetz",
    label: "LDG NRW",
    sourceId: "ldg-nrw",
    parser: ldgNrwParser,
  },
...
async function upsertSource(sourceKey: string, title: string, shortName: string | null, officialUrl: string, versionLabel: string | null): Promise<string> {
  ...
  const { error } = await supabase.from("legal_sources").upsert({
    ...
    official_url: officialUrl, jurisdiction: sourceKey === "beamtstg" ? "Bund" : "NRW",
    authority: sourceKey === "beamtstg" ? "Bundesrepublik Deutschland" : "Land Nordrhein-Westfalen",
    federal_state: sourceKey === "beamtstg" ? null : "NRW",
    legal_domain: "Dienstrecht", version_label: versionLabel, official_source: true, authority_verified: true,
...
async function importOne(target: (typeof TARGETS)[number], authToken: string): Promise<void> {
  ...
  const rowId = await upsertSource(target.sourceId, document.source.title, document.source.shortName, target.url, document.version.label);
